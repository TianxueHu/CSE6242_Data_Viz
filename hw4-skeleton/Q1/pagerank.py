...
"""
def author():                                                                                             
        return 'thu82' # replace gburdell3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903519328 # replace with your GT ID number      
...
class PageRank:
    ...
    def run_pagerank(self, node_weights,  damping_factor=0.85, iterations=10):
        ...
        for it in range(iterations):
            ...
            tmp = [0.0] * (self.max_node_id + 1)
            for source, target in self.read_edge_file(self.edge_file):

                tmp[target] += scores[source]/self.node_degree[source] 
           
            for index in range(self.max_node_id + 1):
                new_scores[index] = (1 - damping_factor) * node_weights[index] + damping_factor * tmp[index]
